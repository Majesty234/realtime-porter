...
module.exports = function(router) {
    ...
    // Route to get all product items for shop page
    router.get('/product', function(req, res) {
        Product.find({}).select('imagePath description title price').exec(function(err, items) {
            if (err) {
                res.json({ success: false, message: err });
            } else {
                if (!items) {
    ...
    router.get('/products', function(req, res) {
        Product.find({}).select('imagePath description title price').exec(function(err, productList) {
            if (err) {
                res.json({ success: false, message: err });
            } else {
                if (!productList) {
    ...
    router.get('/editproduct/:id', function(req, res) {
        ...
        Product.findOne({id: req._id}, function(err, product) {
            if (err) {
                res.json({ success: false, message: 'Something went wrong in attempt to edit product ' + err });
            } else if(!product){
                res.json({ success: false, message: 'Sorry, product not found' });
            } else {
