...
.controller('productListCtrl', function($scope, $routeParams, Product){
    var app = this;
    ...
    function getProductLists() {
        Product.getProductLists().then(function(data) {
            if (data.data.success) {
                app.productList = data.data.productList;
                app.loading = false;
    ...
    // Function: get the product that needs to be edited
    app.showEditProductModal = function() {
        Product.editProduct($routeParams.id).then(function(data) {
            // Check if the user's _id was found in database
            if (data.data.success) {
                $scope.newImg = data.data.product.imagePath; // Display image in scope
                $scope.newTitle = data.data.product.title; // Display product's title in scope
                $scope.newDescription = data.data.product.description; // Display discriptin in scope
                $scope.newPrice = data.data.product.price; // Display user's permission in scope
                app.currentProduct = data.data.product._id; // Get product's _id for update functions
